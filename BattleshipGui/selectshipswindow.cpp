...
SelectShipsWindow::SelectShipsWindow(int* choice, QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::SelectShipsWindow)
{
    ui->setupUi(this);
...
void SelectShipsWindow::on_submit_released()
{
    ...
    for(int i = 0; i < 7; i++)
    {
        if(u.checkIfValid(s[i].getY(),s[i].getX(),s[i].getDirection(), s[i].getSize()))
            getUser()->getGrid().addShip( s[i].getSize(), s[i].getDirection(), s[i].getY(), s[i].getX());
        else
             check = false;
    }
    ...
    if(check == false)
    {
        ...
        {
        qDebug() << "pvp";
        PvpWindow *pvp = new PvpWindow(getUser(), this);
        pvp->show();
        this->hide();
        }
        else {
            PvbWindow *pvb = new PvbWindow(getUser(), this);
            pvb->show();
            this->hide();
            qDebug() << "pvp";
