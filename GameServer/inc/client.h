...
private:
    ...
    inline void set_status(unsigned char stat) {
        status.store(stat, std::memory_order_relaxed);
    }
...
public:
    ...
    /**
     * @return the ID of the game this client is in, or 0.
     */
     ...
     */
    inline void set_in_game(const char g_id) {
        set_status(STATUS_IN_GAME);
        game_id = g_id;
    }
    ...
    inline bool is_searching() {
        return get_status() & STATUS_WAITING;
    }
