...
private:
    ...
    ts_queue<std::weak_ptr<client>>& c_queue;
    ts_queue<message*>& m_queue;
    client_pool& c_pool;
    const unsigned int tout_count_limit;

    /* Message Producer message-type handler helper methods. */
    bool handle_receive_error(const message* msg,
        const shared_ptr<client> msg_client);
...
public:
     ...
     * @param msg_queue
     */
    client_message_producer(ts_queue<std::weak_ptr<client>>& c_queue,
