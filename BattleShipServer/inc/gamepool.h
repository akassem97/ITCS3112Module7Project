...
private:
    std::vector<std::shared_ptr<game>> pool; // holds pointers to clients
    std::list<unsigned char> empty_queue; // holds empty pool indexes
    const unsigned char max_game_count; // max number of clients in pool
    unsigned char game_count; // number of clients in pool
...
public:
    ...
    //
    std::shared_ptr<game> create_and_add_game(
    ...
    //
    const std::shared_ptr<game> get(const unsigned char game_id) const;
